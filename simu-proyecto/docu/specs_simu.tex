...
El funcionamiento del simulador se explica a continuación:
\begin{figure}[ht]
	\centering
	\begin{tikzpicture}
		...
		\node[block, left=1.3cm of mcu] (esc1) {HB 1};
		\node[block, right=1.3cm of mcu] (esc2) {HB 2};
		\node[round, draw=black, left=0.5cm of esc1] (m1) {M1};
		\node[round, draw=black, right=0.5cm of esc2] (m2) {M2};
		\node[block, above=0.5cm of mcu] (esc) {ESS};
		\node[above=0.5cm of esc] (vs) {24 V};
		...
		\path[reline] (mcu.west) -- (esc1.east) node[midway, above, font=\footnotesize] {PWM};
		\path[reline] (mcu.east) -- (esc2.west) node[midway, above, font=\footnotesize] {PWM};
		\path[reline] (control.east) -- (logger.west);
		\path[line, draw=red] (vs.south) -- (esc.north);
		\path[line, draw=red] (esc.west) -| (esc1.north);
		\path[line, draw=red] (esc.east) -| (esc2.north);
		\path[line, draw=red, -] ([yshift=1mm]esc1.west) -- ([yshift=1mm]m1.east);
		\path[line, -] ([yshift=-1mm]esc1.west) -- ([yshift=-1mm]m1.east);
		\path[line, draw=red, -] ([yshift=1mm]esc2.east) -- ([yshift=1mm]m2.west);
		\path[line, -] ([yshift=-1mm]esc2.east) -- ([yshift=-1mm]m2.west);
	\end{tikzpicture}
\end{figure}
...
Se ha considerado como actuadores a todo el conjunto de puente H, motor y encoder magnético que está duplicado en el simulador. Cuando está conectado todo, el puente H recibe una señal PWM del microcontrolador que determina el par (y la velocidad) con la que el motor busca alcanzar la posición comandada.
\par Los sensores magnéticos utilizados son los MT6701, dada su elevada precisión (14 bits), su alta disponibilidad y la familiaridad con su uso.
