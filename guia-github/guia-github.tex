...
\end{lstlisting}
    Donde el link es el link del repositorio, por lo que \texttt{usuario} en este caso es el usuario dueño del repositorio. Si es la primera vez, puede que git pida iniciar sesión.
...
Que se encargará de subir todos los \textbf{commits} nuevos al repositorio remoto. 
...
\begin{lstlisting}[style=powershell]
    git add "archivo"
\end{lstlisting}
Pasar a este comando el punto, \texttt{git add .} sirve para indicar que se deben añadir al Staging Area todos los archivos que hayan sido modificados del directorio. Cuando se ejecuta
\begin{lstlisting}[style=powershell]
    git commit -m "mensaje"
\end{lstlisting}
Se genera una nueva versión en el repositorio local, con todo lo que había entonces en el Staging Area.

\subsubsection{Quitar archivos del Staging Area}
Para eliminar archivos del Staging Area antes de hacer un commit, se puede utilizar el comando:
\begin{lstlisting}[style=powershell]
    git rm --cached nombre/archivo
\end{lstlisting}
GitHub tiene un límite de 100 MB para el tamaño de los archivos. Si hay un archivo que exceda este tamaño en el repositorio local, el comando \texttt{git push} fallará, con el mensaje correspondiente de que el archivo mencionado excede el límite de tamaño. Por esto, puede ser interesante quitar estos archivos del Staging Area.
...
\begin{lstlisting}[style=powershell]
    git log --oneline
\end{lstlisting}
En este caso, se ha añadido la etiqueta \texttt{--oneline} para que aparezca por línea la ID del commit y su mensaje. Con esto, se pueden copiar las ID's de cada commit para, por ejemplo poder resetear el Staging Area o los cambios a un estado anterior.
\par Sin embargo, si ha llegado a saltar el mensaje de error al hacer el \texttt{git push}, esto quiere decir que ya existe un commit con los cambios que incluyen el archivo grande. Los commits guardan cambios respecto al commit anterior, y ya se ha guardado localmente un commit con este archivo, lo único que se puede hacer es resetear el historial de commits hasta justo antes de este commit erróneo.

\subsection{Eliminar commits erróneos}
...
\end{lstlisting}
Donde \texttt{type} puede ser \texttt{hard, mixed} o \texttt{soft}, y \texttt{commit-ID} es la ID del commit al que se quiere resetear. A continuación se explica la diferencia (a nivel práctico) entre los distintos modos de \texttt{git reset}.
...
\subsubsection{--mixed}
Esta opción es igual que la \texttt{--soft}, es decir, que no se modificarán los archivos dentro del repositorio local, pero sí que se sobreescribe el Staging Area con la del commit al que se retrocede, por lo que si se quieren guardar los cambios que se han hecho hasta ahora, habrá que volver a escribir \texttt{git add .}

\subsubsection{Ejemplo solución de commit erróneo}
Supongamos que se está trabajando con Patran-Nastran y se ha generado un archivo \texttt{.f06} muy grande que supera el límite de 100 MB. Una vez alcanzado un hito del trabajo, se ejecutan los comandos:
\begin{lstlisting}[style=powershell]
    git add .
    git commit -m "Trabajo terminado"
    git push
\end{lstlisting}
Y el comando que falla es \texttt{git push}, con el aviso de que hay un archivo que excede el tamaño máximo permitido por GitHub. Si ahora se elimina el archivo \texttt{.f06}, y se vuelve a hacer \texttt{git add.}, \texttt{git commit} y \texttt{git push}, seguirá habiendo errores, porque el commit anterior en este caso sigue conteniendo el archivo grande.
\par La forma de solucionar esto, evitando perder todo el progreso conseguido, es utilizando \texttt{git reset --mixed}. El flujo de trabajo sería:
\begin{lstlisting}[style=powershell]
...
\begin{enumerate}
    \item Resetear para volver a un commit donde no existe el archivo .f06 (o existe una versión anterior no tan grande)
    \item Añadir al Staging Area todo
    \item Eliminar del Staging Area los archivos \texttt{.f06}
    \item Commit estándar
    \item Push estándar
\end{enumerate}

\subsection{Archivo .gitignore}
Cuando se está trabajando con programas que generan archivos muy grandes (como Patran-Nastran o Ansys), o se generan carpetas de \textit{builds} que son específicas para cada máquina y no deben compartirse en el repositorio, la forma más cómoda de evitar que estos archivos entren al Staging Area y a los commits es diciéndo a git que ignore los cambios en estos archivos o carpetas, y que no los añada nunca al repositorio remoto.
\par La forma de hacer esto es con un archivo \texttt{.gitignore}, que se encarga de especificar qué archivos ignorar. Además, soporta sintaxis para seleccionar archivos por extensión o carpetas/directorios por nombre.
\par Para incluir esto, simplemente hay que crear un documento de texto en el editor de preferencia con nombre y extensión simplemente \texttt{.gitignore} (el punto es importante). A continuación se presenta un ejemplo de lo que podría ser interesante añadir para estudiantes de grado y máster en la ETSIAE:
...
\subsubsection{Cómo abrir una rama nueva}
...
\begin{enumerate}
    ...
    \item Generaremos el \textit{token} y en la pantalla a continuación, es muy importante copiarlo y guardarlo en un sitio seguro, ya que esta será la contraseña que siempre se debe usar para iniciar sesión en una máquina remota.
\end{enumerate}
...
\section{Colaboradores en GitHub}
Cuando varias personas van a trabajar de forma colaborativa en un repositiorio, GitHub proporciona dos formas distintas de facilitar este tipo de trabajo. Ahora se explicará lo sencillo que es añadir colaboradores en un repositorio.

\subsection{Requisitos previos}
Es necesario que:
\begin{itemize}
    \item El repositorio exista (no necesariamente que contenga nada).
    \item Todos los colaboradores tengan una cuenta de GitHub.
\end{itemize}

\subsection{Añadir a colaboradores}
El propietario del repositorio deberá:
\begin{itemize}
    \item Ir a los ajustes del repositorio $\to$ colaboradores $\to$ añadir gente
    \item Escribir la dirección de correo electrónico asociada con la cuenta de un colaborador o su nombre de usuario.
    \item Hacer click sobre añadir `nombre-usuario'.
\end{itemize}
El colaborador deberá entonces aceptar la solicitud de colaboración. Una vez hecho esto, el propietario podrá ajustar el nivel de permisos de cada uno de los colaboradoes. Para permitir la contribución, deberá seleccionarse \textbf{read/write}.
\end{document}
